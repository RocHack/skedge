...
$(document).ready(function(){
  ...
  if (!mobilecheck()) {
    $(document).scroll(function() {
      ...
      if (top >= 20) {
        $('.head-fixed').css('border-bottom',"1px solid #CBCBCB");
      }
      else {
        $('.head-fixed').css('border-bottom',"none");
      }
    });
  }
...
$(document).on("click", "a, button, input[type=submit]", function (e) {
  ...
  var name = $target.attr("name") ? $target.attr("name") : $target.parent().attr("name");
  if (name)
  {
    var properties = {};
...
$(document).on("submit", "form", function (e) {
  var $target = $(e.currentTarget);
  var properties = $target.serializeArray().reduce(function(obj, item) {
      obj[item.name] = item.value;
      return obj;
  }, {});
  properties.page = window.location.href;
